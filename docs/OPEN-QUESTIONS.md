...
| Q16 | Are parameter *values* (filled data) part of this plugin? | Yes in-core / host plugins only / later phase | **Strong lean with Composition:** **ParameterValue** (and CompositionRow cells) are **in-core** — otherwise Zusammenstellung has no content. Host may still own price/stock. WP mapping = Q11. | open |

...
| Q14 | Is a parameter always assigned to exactly one node? | Always one owning node / can be shared / taxonomy-level / other | **Dropped (entfällt):** Parameter *is* a Node (Q33); placement via hierarchy/`contains` and/or other Relations (Q54) — no separate `node_id` | decided |
...
| Q52 | How do **list** / **table** / **enum** relate (Collection model)? | Separate types / **Collection** super-kind (list=1 col, table=n cols, enum=closed list) / enum stays apart | **Decided:** Collection → `list` \| `table` \| `enum`. list ≡ 1-column table; enum created like list (1 typed column + closed option children under that column). Kind binding = Q53. | decided |
| Q53 | How is Collection **kind** bound for a concrete type (e.g. `my_list`, `Bauart`)? | Parent under list/table/enum / Relation `has_type` → kind / XOR / **has_type only** | **Decided:** kind **only** via `─[has_type]→ list\|table\|enum`. Parentage never confers kind. Column/option membership = hierarchy Relation (`contains`, Q54) — same edge table. | decided |
| Q54 | Is tree `parent_id` semantic, or only UI/org — should hierarchy be a Relation too? | `parent_id` semantic containment / org-only + duplicate Relations / **hierarchy = RelationType** (`contains`) and tree is a view / hybrid | **Decided:** hierarchy uses the **same Edge/Relation table** as other links (rename Relations→Edges optional). RelationType e.g. `contains` for containment; tree UI = projection. `parent_id` if kept = **denormalized cache only**, not a second semantic system. | decided |

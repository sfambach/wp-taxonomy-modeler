...
| Q52 | How do **list** / **table** / **enum** relate (Collection model)? | Separate types / **Collection** super-kind (list=1 col, table=n cols, enum=closed list) / enum stays apart | **Decided:** Collection → `list` \| `table` \| `enum`. list ≡ 1-column table; enum created like list (1 typed column + closed option children under that column). Kind binding still Q53 (XOR). | decided |
| Q53 | How is Collection **kind** bound for a concrete type (e.g. `my_list`, `Bauart`)? | Parent under list/table/enum / Relation `has_type` → kind / XOR / **has_type only** (tree org-only) | **Strong lean: `has_type` → list\|table\|enum is authoritative.** Parent under a Collection kind is **organizational only** — does **not** confer kind (avoids tree/edge mixup). See hierarchy-vs-Relations spin | open |
| Q54 | Is tree `parent_id` semantic, or only UI/org — should hierarchy be a Relation too? | `parent_id` semantic containment / org-only + duplicate Relations / **hierarchy = RelationType** (`contains`) and tree is a view / hybrid | **Exploring:** object “cloud” + typed edges; tree chrome via DisplayHint. Org-only `parent_id` forces double links (list→column). Prefer hierarchy-as-Relation long-term (Q35). | open |

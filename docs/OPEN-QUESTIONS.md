...
| Q51 | How do Basiseinheit and Präfix relate, and where is the scale factor (×1000)? | Unit─[allows_prefix]→Präfix + factor on Präfix / factor on edge / hybrid | **Agreed direction:** Relations = allowed prefixes; **factor on Präfix Node**; UI select gets Basiseinheit → **generates** choices (base + prefix×unit labels, e.g. Ohm/kOhm) — no stored `kOhm` Nodes. “Kinder” = Relation targets, not tree children under Ohm | open |

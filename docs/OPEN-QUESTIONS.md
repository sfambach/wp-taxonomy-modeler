...
| Q56 | What is a Composition vs a catalog Bauteil? | GPU/Widerstand are Compositions / **only lists are Compositions** / host-only | **Corrected lean:** **Bauteil** = catalog part (Widerstand, GPU-Karte) with Parameters. **Composition** = Zusammenstellung (BOM/Rezept/Build) with columns+rows; Bauteile appear **only via Bauteil-Ref column**. Naming: UX Zusammenstellung / internal Composition. | open |

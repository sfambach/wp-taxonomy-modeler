...
| Q53 | How is Collection **kind** bound for a concrete type (e.g. `my_list`, `Bauart`)? | Parent under list/table/enum / Relation `has_type` → kind / XOR / other | **Fresh start** under design guidelines (clear structures; named objects when better). Prior TE closed — no carry-over. | open |

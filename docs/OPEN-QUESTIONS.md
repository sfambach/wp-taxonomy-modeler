...
| Q51 | How do Basiseinheit and Präfix relate, and where is the scale factor (×1000)? | Unit─[allows_prefix]→Präfix + factor on Präfix / factor on edge / hybrid | **Agreed direction:** Relations for **allowed** prefixes (UI filter); **factor on Präfix Node** (SI: kilo=1000); edge factor only as override. Fits Nodes + Relation + Node.config — no new class. Prefer “factor” not “Multiplizität” | open |

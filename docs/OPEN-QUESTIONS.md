...
| Q52 | How do **list** / **table** / **enum** relate (Collection model)? | Separate types / **Collection** super-kind (list=1 col, table=n cols, enum=closed list) / enum stays apart | **Exploring:** Collection → list \| table \| enum; **enum created like list** (1 typed column + closed option children). See data-structure spin | open |
| Q53 | How is Collection **kind** bound for a concrete type (e.g. `my_list`, `Bauart`)? | Parent under list/table/enum branch / Relation `has_type` → kind / either but **XOR** | **Strong lean: XOR** — parentage under kind **or** `has_type`→kind, never both. Columns each need their own `has_type` (list/enum: exactly one column node) | open |

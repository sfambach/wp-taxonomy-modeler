...
| Q53 | How is Collection **kind** bound for a concrete type (e.g. `my_list`, `Bauart`)? | Parent under list/table/enum / Relation `has_type` → kind / XOR / other | **Fresh start** — prior “has_type only + contains edges” branch closed as thought experiment (see data-structure). No carry-over decision. | open |
| Q54 | How do tree hierarchy and Relations relate? | Semantic `parent_id` / org-only tree / hierarchy as Relation / hybrid | **Fresh start** — prior branch closed as TE. **Out of that branch:** `parent_id` as cache of hierarchy edges (rejected). Restart without that hybrid. | open |

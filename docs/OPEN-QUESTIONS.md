...
| Q51 | How do Basiseinheit and Präfix relate, and where is the scale factor (×1000)? | Unit─[allows_prefix]→Präfix + multiplikator Relation / config on Präfix / factor only on allows_prefix edge | **Agreed direction:** `allows_prefix` = allowed set (per unit; Farad without k/M); **Präfix ─[multiplikator]→ int** with `props.value`; UI derives Ohm/kOhm/…; forward+back convert via same factor | open |

...
| Q16 | Are parameter *values* (filled data) part of this plugin? | Yes in-core / host plugins only / later phase | **Strong lean:** **ParameterValue** on **Bauteile** and in **CompositionRow** cells are **in-core**. Host may still own price/stock. WP mapping = Q11. | open |
...
| Q26 | Must type/prefix/base_unit Nodes be children of Type/Präfix/Basiseinheit? | Strict branch check / any node | **Decided:** type of a Node is resolved **only under the Type branch** (`Project.type_node` / Datentypen). Präfix / Basiseinheit likewise only under their anchors — never under Compositionen or Bauteile. | decided |
...
| Q55 | How are catalog **Parameters** defined and inherited (e.g. Wert + Bauform on Widerstand)? | Parameter object on Node / Parameter as Node role (Q33) / only Relations `consists_of` | **Decided (keeps Q33):** no Parameter *class*. “Parameter” = vocabulary for a **typed slot Node** (`has_type`, optional `config.required` / `ref_scope`). Children **inherit** slot defs along catalog `parent_id`; leaves/instances fill **ParameterValue**. Bauform = slot → enum `Bauart`. | decided |
| Q56 | What is a Composition vs a catalog Bauteil? | GPU/Widerstand are Compositions / **only lists are Compositions** / host-only | **Decided lean:** **Bauteil** = catalog part; **Composition** = Zusammenstellung (columns+rows). Bauteile only via column type **`subtree`** + `ref_scope` (UX: Bauteil-Ref / „Bauteil Wahl“). Naming: UX Zusammenstellung / internal Composition. | decided |
| Q57 | Does a BOM / Composition have a Fußzeile (footer)? | No footer / host-only footer / **Composition has Fußzeile** | **Decided:** a **BOM has a Fußzeile** (e.g. Summe Menge in Stück; price totals may be host). Part of the Composition table view, not an optional decoration. | decided |

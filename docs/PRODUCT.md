...
**Status:** Scaffolding ≈ **`0.0.414`** on **`wtt_fs` Fallstudie** only (`wtt_tree` retired). **Q123 locked** — attributes = `besteht_aus`/`aggregation` Relations (`Settings.data`/`view`); see [`DEVELOPER-ATTRIBUTE-MODEL.md`](DEVELOPER-ATTRIBUTE-MODEL.md). Also: **Q88**/`Q90`/`Q91`/`Q97`/`Q98`/`Q115`; preferred **`embed`**; `node_ref`/`ref_scope` deprecated. Full domain still planning. **Fallstudie = gold scaffold — not Phase-1 sign-off.** Plan **0.7.97**; status stays scaffolding.  
...
- **Pure Template** (**read-only**) = Datentypen (simples; **no** product reliance on catalog `enum`/`list`/`table` — **Q90**; **no** product catalog `node_embed` — **Q72**) + **Präfixe** + **Unit**/{With prefix, Without prefix} under Data Types (**Q120**; no Konstanten folder); editable product examples live under Fallstudie **Model** (BOM / Bauteile) — **not** an Implementation/ SoT and not a parallel `wtt_tree` project.

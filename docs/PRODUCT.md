...
**Status:** Scaffolding ≈ **`0.0.390`** (admin tree on **`wtt_fs` Fallstudie** only — **`wtt_tree` retired**; Relations Q74–Q78; set=`composition` Q75; **Q88** hierarchy datatype = father; **Q90** Complex `enum`/`list`/`table` **parked**; **Q91** Registry + preferred render/converter/validators; **Q95** icons; **Q96** Registry `builtin.*` id bindings; **Q92** / **Q77** chooser = nodes by id; **Q97** Model_Data links + composition cascade; **Q93** host stores **ids only**; **Q98 / UR-S1** Model versions; **Q115** Read-only + Default value (node seeds); **no product Implementation/** — refs → **Model** only; pick+fill = preferred **`embed`** not catalog `node_embed`). Full Project/Node domain still planning. **Fallstudie = gold scaffold shape — not Phase-1 model sign-off.** Plan **0.7.80**; status stays scaffolding.  
...
- **Decided (Q111 revised):** **Bindung** drives storage — **Composition** → embedded in parent (list **Position** only under Model); **Aggregation** → linked Model_Data (default under Model). Scaffold DEFAULT_BINDING = aggregation ≈ **0.0.388**. **Q113:** `enum Renderer` (`EmbeddedRenderer`, …) in progress. **Q114:** Attribute Options share Node Preferred R/C (+ Validators later); render+converter side by side ≈ **0.0.389**.

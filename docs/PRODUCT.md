...
**Status:** Scaffolding ≈ **`0.0.497`** on **`wtt_fs` Fallstudie** only (`wtt_tree` retired). **Q123 locked** — attributes = `besteht_aus`/`aggregation` Relations (`Settings.data`/`view`; own-attr edge SoT); see [`DEVELOPER-ATTRIBUTE-MODEL.md`](DEVELOPER-ATTRIBUTE-MODEL.md). **Settings UI parity** ([`plans/settings-ui-parity.md`](plans/settings-ui-parity.md)): Knoten-/Attribut-Walk = one Settings surface; legacy Options chrome being removed. Modeling UAT still open. Also: **Q88**/`Q90`/`Q91`/`Q97`/`Q98`/`Q115`; preferred **`embed`**. Full domain still planning. **Fallstudie = gold scaffold — not Phase-1 sign-off.** Plan **0.7.124**; status stays scaffolding.  
...
- **Pure Template** (**read-only**) = Datentypen (simples; **no** product reliance on catalog `enum`/`list`/`table` — **Q90**; **no** product catalog `node_embed` — **Q72**) + **Definition/Konstanten** (**Präfixe**, **Basiseinheiten**/{With prefix, Without prefix}, **Bauformen**, **Währung**) + **Data Types** hosts such as **Unit type** — editable product examples live under Fallstudie **Model** (BOM / Bauteile) — **not** an Implementation SoT and not a parallel `wtt_tree` project.
...
- **Eigenschaften (attributes)** = **`besteht_aus` / `aggregation` Relations** with `name` + type target (**Q123**) — not hierarchy children; scaffold slots = debt.
...
- **Decided (Q83 superseded / OQ-B2):** **Model/Bauteil** = kinds **and** part records (by id). **No Implementation/ branch** as product SoT. Line refs → Model only. Pick+fill = preferred render **`embed`**. Catalog **`node_ref` / `node_embed` / `ref_scope` deprecated** (do not use until a use case).
...
- **Decided (Q123 / OQ-W1…W16):** Attributes = **composition/aggregation Relations** (`name` + target) — no slot terms. **`Settings.data`** + **`Settings.view`**; recursive Settings/Render walk; hybrid overrides. Instance keys = Relation id. Attributes panel = wizard. Deprecated: `node_ref` / `ref_scope` / `node_embed`. See [`docs/DEVELOPER-ATTRIBUTE-MODEL.md`](DEVELOPER-ATTRIBUTE-MODEL.md).
- **Decided (Q122):** Type **properties** live on the type; **override along inheritance** — same law everywhere. **Composed** types expose component settings dynamically (feeds the Q123 walk).
- **Decided (Q111 revised):** **Bindung** drives storage — **Composition** → embedded in parent; **Aggregation** → linked Model_Data. Delete cascade matches (composition dies with host; aggregation keeps targets, drops Relation).
- **Decided (Q114):** Attribute Settings UI = **same recursive walk** as node Settings (OQ-W6), not a thinner Options row.
- **Decided (Q126):** Node configuration page = one vertical **`ConfigPageRender`** box stack (Actions → MetaSettings → Bools → Display → Attributes → Preview); same entry everywhere.
...
- **Decided (Q90):** Complex catalog kinds **`enum` / `list` / `table` parked** — not active product types. Closed values → hierarchy inheritance + attributes / Festwerte. **≈ 0.0.463:** Fallstudie soft-trashes `enum`/`list`/`node_*` from the tree; stops seeding them. Legacy **`table`** leaf kept only while BOM::Tabelle binds it. Bauart CatalogChoice under Complex. **CatalogChoice:** flat `<select>` if max depth ≤ 1, else tree chooser; Festwert seeds the value. **Value SoT** → **Q93 decided** (id only on host).
- **Decided (Q91 + Q123):** Node-only domain ≠ one renderer — **Registry + many type-specific renderers**. Preferred R/C live in **`Settings.view`** (hybrid live + Relation override deltas); validators in **`Settings.data`**. Same recursive paint walk as Settings. Same Registry / object surfaces across admin ↔ block ↔ frontend (parity). Scaffold `_wtt_preferred_*` seed = debt.

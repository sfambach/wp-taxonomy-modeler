...
- **Decided (Q90):** Complex catalog kinds **`enum` / `list` / `table` parked** — not active product types. Closed values → hierarchy inheritance + attributes / Festwerte. Scaffold may still show leftover Complex leaves / Enum UI / collection-table until removal.

...
✅ **Settled 2026-08-23 by [D-209](90-decision-log.md):** `long` is gone — it was renamed
`help`, and the fallback chain ends there. Everything [D-151](90-decision-log.md) and
[D-153](90-decision-log.md) say about `long` now says `help`, so the chain reads
`<role>·<number>` → `<role>·one` → `help·one` → `node.name`. ⚠️ *The gap was mine: an anchor
was retired without checking what hung from it.*

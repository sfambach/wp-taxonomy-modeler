...
**Status:** Target architecture — domain model still planning; **scaffold ≈ `0.0.562`** on **`wtt_fs` (Fallstudie)** only (`wtt_tree` retired). **Q123 locked** — Relation-only attributes, `Settings.data`/`view`, recursive walk ([`DEVELOPER-ATTRIBUTE-MODEL.md`](DEVELOPER-ATTRIBUTE-MODEL.md)); migrate remaps slots → named edges; `Settings_Walk` + own-attr edge SoT + **Walk Settings** (≈ 0.0.441+; nested **table chrome** + deferred Choices ≈ **0.0.531**): path-keyed deltas on the attribute Relation; depth-0 Default → `edge.default`; admin Preview = **host Preferred only**; **nested attribute cells** = walk / type Preferred (`paintStructureEmbed` Composition / `paintAggregationBound` Aggregation chooser/bind — ≈ **0.0.563** + `typePresentation` Q117 — ≈ **0.0.558**); **Compact Hide/RO by edge id** (not shared Simple `typeId`) ≈ **0.0.559**; **Hide remount shell** = CSS flex `#wpwrap`→`#wtt-app` + clear inline sizes / pin admin scrollers (≈ **0.0.562**; do not measure rects for pixel heights); **Settings cascade → paint locked** (plan **0.7.140** — presets drive paint; no display-name hardcoding); structure hosts never list heirs as CatalogChoice; Preferred default = **type node** (no display-name heuristics — debt to finish stripping). **Q126:** ConfigPage without `childNodes` box — Child-list Choices under Display when Preferred = `ChildListRenderer` (≈ **0.0.532**); **ChildList Choices/Default follow Q90 depth** (≤1 list, ≥2 tree — ≈ **0.0.541**). **Properties scroll lock** (≈ **0.0.542**). **Basiseinheit leaves Preferred = Compact** (≈ **0.0.543**); Compact **`_wtt_compact_show_labels`** (Show field labels, ≈ **0.0.555**). **MultistepRenderer** (ex-Embedded; dialog|inline ≈ **0.0.546**; Bindung Phase B ≈ **0.0.552**; kind Preferred ≈ **0.0.554**; Composition Display sync ≈ **0.0.555**). **Multistep Phase A kind pick** follows Q90 (≤1 ListChooser / ≥2 TreeChooser; chooser root not painted as a row, ≈ **0.0.548**). **Settings_Walk** includes **child_of-inherited** attributes (Organisation = Kontakt heirs, ≈ **0.0.551**). **Admin: one scrollbar** (pane scroll only, ≈ **0.0.550**; Hide-toggle hold ≈ **0.0.556**). **Attribute Preferred paint** — `instanceFromView` / `fieldFromProp` copy Preferred + Q117 `presentationConfig`/`hostPresentation` (≈ **0.0.549–0.0.550**); Multistep before Structure-Embed. **Q117 node_presentation** respects Presentation map + context (empty draft must not shadow server map; form/select/help name only as empty-slot fallback, ≈ **0.0.557**); nested structure embeds use **`typePresentation`** of the attribute type (not outer host, ≈ **0.0.558**). **Hide → Preview filter** (≈ **0.0.556**). **Settings walk summary always includes depth-0** (fixes empty Options “Loading…” when `nodeCount=1`, ≈ **0.0.547**). **Implementation/BOM seed retired** (≈ **0.0.533**). **Display Preferred form stack** (≈ **0.0.536**). **Konstanten/Präfixe locked** (≈ **0.0.540**). **Q125 `calc`**. Q90 parked `enum`/`list`/`table` / `node_*`. **Gold = Fallstudie**; status stays scaffolding. Parity matrix: [`plans/settings-ui-parity.md`](plans/settings-ui-parity.md).
...
- **Settings cascade → paint (locked ≈ 0.0.558 / plan 0.7.140):** Defaults cascade detail→model (illustration: Präfix → Basiseinheit → Unit type → Model). Preview and nested cells **must** honour Preferred, Q117 Presentation, Hide, Mult, Bindung, and walk/Relation overrides. **No** hardcoding by display name / path / special-case node. Hardcode **only** inside a renderer for that renderer’s own chrome, **current layer only**; nested cells use the nested type’s Preferred (`paintStructureEmbed` / `resolveFieldPreferredPaint` + `typePresentation`). Walk Hide/RO onto nested `typeProperties` matches by **attribute edge id** only (≈ **0.0.559** — never shared Simple `typeId`). See [Presentation surfaces — Settings cascade](plans/project-plan.md#settings-cascade--paint-locked--00558--plan-07140).
- **Walk Preferred chrome (≈ 0.0.559):** Attribute / Settings walk Render → Converter → Validators uses the same **stacked** order as node Display Preferred (`WTTSettingsRender.renderPreferredStack`); narrower column OK. Do not revive side-by-side triple grids for walk.
...
- **Preferred render** (`_wtt_preferred_render`, ≈ 0.0.358+): object layouts (`FormRenderer`|`TableRenderer`|`CompactRenderer`|`CompactVerticalRenderer`|`MultistepRenderer`|`ChildListRenderer`) **or** field Registry renderer ids (`int`, `bool`, …). Legacy wire `EmbeddedRenderer` / `embed` / `pick-fill` normalize to **`MultistepRenderer`** (≈ **0.0.546**). UI lists **only** options that `canRender` the node. Admin preview / Object View `layout=auto` use preferred; **Multistep** = pick kind → filter/create (UR-B6); **`ChildListRenderer`** = list of hierarchy children (default for Konstanten hosts with children, ≈ 0.0.483). **Display ConfigPage (≈ 0.0.536):** stacked form rows — Render / Converter / Validators (label left, select right); renderer options (ChildList Default/Choices; Multistep **dialog|inline** ≈ **0.0.546**) mount **under** Render; converter options hook under Converter; validators table only when entries exist (no empty stub). **ChildList Default/Choices (≈ 0.0.541):** Q90 depth ≤ 1 → flat; ≥ 2 → Choices tree + TreeChooser Default. Attribute walk may still use side-by-side Preferred chrome until converged.
...
Same depth law applies to **ChildListRenderer** ConfigPage **Choices** (checkbox tree) and **Default** (ListChooser vs TreeChooser) ≈ **0.0.541** — e.g. Präfixe (depth 1, flat) vs Basiseinheiten → With/Without prefix → units (depth ≥ 2, tree).
...
**Value SoT (Q93 / Q97 / Q111 revised):** **Bindung** = storage: **Composition** → embedded values/rows in the parent instance bag; **Aggregation** → linked Model_Data + `links[]` (id on host; fill on referenced instance where applicable). Preferred **`MultistepRenderer`** = multi-step chrome (**Q112**); **Phase B follows Bindung** (Composition = step fill in context; Aggregation = list/search/create/bind — ≈ **0.0.552**). Mode dialog|inline is renderer-local — not Settings picker presentation (Q74/Q84).
...
**Multistep pick+create (UR-B6; Preferred `MultistepRenderer`, ≈ 0.0.546; Bindung chrome ≈ **0.0.552` / kind Preferred paint ≈ **0.0.554` / Composition Display ≈ **0.0.555` / Aggregation structure cells ≈ **0.0.563`):** Phase A = kind under branch (Q90 list|tree). **Concrete Aggregation type** (no specialization catalog, e.g. Organisation) **skips Phase A** → Phase B on `typeId`. **Phase B depends on Bindung (Q111):** **Composition** → fill kind attributes with the **kind node’s Preferred** chrome — values in parent/context. **Aggregation** → filter/search Model_Data + create + bind instance id (Q93). Structure cells with Bindung=aggregation: unbound Editable = Multistep chooser (never composition Name/E-Mail embed); bound = type Preferred Display readonly + Change. Aggregation targets without specialization children may use the type itself as sole kind (≈ 0.0.531). Extend TreeChooser + ModelInstancePicker — no third picker.  
...
**Quantity / Unit (Q120 + Q91 — scaffold ≈ `0.0.411`, leaf Preferred ≈ `0.0.543`):** Same law — not a second concept. **Basiseinheit catalog leaves** Prefer **`CompactRenderer`** over inherited Praefix?/Kuerzel attrs (definition chrome). **UnitRenderer** = Prefix? + Symbol for **quantity usage** fields (unit↔prefix marriage). **QuantityRenderer** = one-row box that calls Value (e.g. Double) + Unit renderers. Do not hardcode Typ/Praefix/Kuerzel in tree-admin; Prefer Registry. Catalog folders (With/Without prefix) remain browse knots / CatalogChoice roots.

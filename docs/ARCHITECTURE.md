...
**BOM Referenz / RefDes (Q47 UR, planning):** UX prefers compact notation (`R1`, `R1,R4,R6`, `R1-R5`, `R1-R5, R8`); **canonical store** = expanded position list `string[]` so an interactive BOM can highlight board placements when a line/part is selected. Menge ↔ count(positions) (Q58). Planned model: Platine → Version → Bauteilliste → Position; `Wert` = Bauteil catalog. See [`MODEL-CATALOG.md`](MODEL-CATALOG.md) (**UR** = User Requirement).  

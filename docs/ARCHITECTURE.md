...
**Object View block:** whole-node chrome (`Object_Render` / `WTTObjectRender.mount`) binds a live node. Canonical layout (**Form + Table auto**): horizontal readonly meta strip (Name / Description / Type) → Form for attributes with Mult ≤ 1 → Table for Mult many (`0..*` / `1..*`). Overrides: Table (all) / Compact.

...
**Status:** Target architecture — domain model still planning; **scaffold ≈ `0.0.414`** on **`wtt_fs` (Fallstudie)** only (`wtt_tree` retired). **Q123 locked** — Relation-only attributes, `Settings.data`/`view`, recursive walk ([`DEVELOPER-ATTRIBUTE-MODEL.md`](DEVELOPER-ATTRIBUTE-MODEL.md)); scaffold slots = migrate debt. Also: Relations Q74–Q78, **Q88**/`Q90`/`Q91`/`Q92`/`Q97`/`Q98`, preferred **`embed`**, `node_ref`/`ref_scope` deprecated. **Gold = Fallstudie** (plan **0.7.97**); status stays scaffolding.
...
**Attribute Options UI (scaffold):** Attributes table row has foldable **Options** (`wtt-attributes__options-toggle`) — preferred render/converter, date/int extras, etc. (`typeExtras`). Target direction: Settings capsule on the composition/aggregation Relation (Q123).
